...
export function PlayerDot({
  ...
  // react-draggable needs a stable ref in React strict/client rendering.
  const nodeRef = useRef<HTMLButtonElement>(null);
  const [isDragging, setIsDragging] = useState(false);
  const markerPixels = useMemo(
    // Convert the court point into a top-left marker position for dragging.
  ...
  const handleDrag = useCallback(
    (_event: DraggableEvent, data: DraggableData) => {
      if (!courtSize.width || !courtSize.height) {
        // Do nothing until ResizeObserver has measured the court.
        return;
      }

      // Convert the dragged marker back into court feet for sandbox analytics.
      ...
      // Shooter dots can snap to common spots; defenders always move freely.
  ...
  return (
    ...
    >
      ...
      >
        {quality ? (
          // The shooter ring reflects shot quality as the point moves.
          <motion.span
        ...
        {isClosest ? (
          // Highlight the defender currently driving the pressure calculation.
          <motion.span
