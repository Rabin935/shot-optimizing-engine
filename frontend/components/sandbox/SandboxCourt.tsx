...
export function SandboxCourt({
  ...
  // Measure the rendered court so feet-to-pixels conversion stays accurate.
  ...
  useEffect(() => {
    ...
    if (!court) {
      // Wait until the DOM node exists before creating ResizeObserver.
      return;
    }

    const updateSize = () => {
      // Store current pixel dimensions for SVG overlays and draggable bounds.
  ...
  const activeSnap = useMemo(() => {
    // Show the snap target only when the shooter is essentially on top of it.
  ...
  const handleDefenderDrag = useCallback(
    (id: string, point: CourtPoint) => {
      // Preserve defender id while sending updated court coordinates upward.
...
function CourtAnalyticsOverlay({
  ...
  if (!visible) {
    // Hide all analytics overlay SVG when the toggle is disabled.
    return null;
  }

  // Convert main points to pixels for overlay lines.
  ...
  const closestDefender = defenders.reduce<DefenderDistance | null>(
    // Recalculate closest defender for overlay highlighting.
    (closest, defender) => {
  ...
  return (
    <svg className="pointer-events-none absolute inset-0 z-[7] h-full w-full" aria-hidden="true">
      ...
      {defenders.map((defender) => {
        // Draw pressure relationship lines from shooter to each defender.
      ...
      {SNAP_TARGETS.map((target) => {
        // Render visual anchors for common shot locations.
...
function MetricPill({
  ...
  // Small readout pills summarize the current court state below the court.
  return (
...
function formatDistance(distance: number) {
  // Infinite distance means the active scenario has no defender.
