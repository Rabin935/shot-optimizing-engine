...
export function StatsPanel({
  ...
  // Prefer backend prediction values, but fall back to local sandbox stats.
...
function getDisplayRecommendation(
  ...
  if (isLoading) {
    // Show a temporary recommendation while the backend request is in flight.
    return {
  ...
  if (!prediction) {
    // Without backend data, use the local sandbox recommendation.
    return fallback;
  }

  // Convert backend recommendation text into the frontend card format.
  return {
...
function normalizeShotQuality(
  ...
  // Accept only quality labels the UI knows how to style.
  if (
    quality === "Excellent" ||
    quality === "Good" ||
    quality === "Average" ||
...
function getRecommendationTone(quality: ShotQuality): ShotRecommendation["tone"] {
  // Map shot quality into the card color family.
...
function getConfidenceTone(
  ...
  // Convert backend confidence labels into stat tile tones.
  if (confidence === "High") {
    return "green";
  }
...
function getPredictionSourceLabel(source: string | undefined) {
  // Display a user-friendly label for the active prediction source.
  if (source === "ml_model") {
    return "ML Model";
  }
...
function HeroMetric({
  ...
  // Hero metrics are the large probability and EPPS cards.
  return (
    <motion.div
      layout
...
function StatTile({
  ...
  // Stat tiles show compact contextual values below the hero cards.
  return (
    <motion.div
      layout
...
function DebugRow({ label, value }: { label: string; value: string }) {
  // Debug rows expose raw values used to build the prediction request.
  return (
...
function formatDistance(distance: number) {
  // Infinite distance means no defender was active in the sandbox.
