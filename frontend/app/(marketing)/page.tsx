...
const navLinks = [
  // Anchor links jump to the main sections on the marketing homepage.
...
const featureCards: {
  ...
  // Feature cards define the reusable content blocks in the features section.
  {
    icon: Activity,
...
const howItWorksSteps = [
  // Workflow steps explain the analytics flow from input to recommendation.
  {
    icon: Move,
...
const techStack = [
  // Tech badges summarize the main tools used across frontend and backend.
...
const heroMetrics = [
  // Mock hero metrics preview the kind of output the simulator produces.
...
const particleClasses = [
  // Lightweight decorative particles are driven by static Tailwind classes.
...
export default function Home() {
  // Marketing homepage introduces ShotOptix and routes users to the sandbox.
  return (
...
function SectionIntro({
  ...
  // Shared section header keeps long landing sections visually consistent.
  return (
    <motion.div
...
function PoseSimulatorMockup() {
  // Hero mockup previews pose analytics without requiring live simulator state.
  return (
    <motion.div
...
function PoseSceneSvg() {
  // SVG illustration represents the shooter, defender, and shot trajectory.
  return (
    <svg
...
function MetricBar({
  ...
  // Render a compact labeled progress bar for demo metric panels.
  return (
    <div>
...
function PoseCourtDemo() {
  // Demo section combines mechanics controls with a court-style visual preview.
  return (
    <motion.div
      initial={{ opacity: 0, y: 28 }}
      whileInView={{ opacity: 1, y: 0 }}
...
function CourtFigureSvg() {
  // SVG court figure shows how pose, pressure, and EPPS connect visually.
  return (
    <svg
      className="absolute inset-0 h-full w-full"
      viewBox="0 0 920 500"
...
function PoseJointsIcon({ className = "" }: { className?: string }) {
  // Custom icon highlights body-joint mechanics for the main feature card.
  return (
    <svg
      viewBox="0 0 48 48"
      fill="none"
