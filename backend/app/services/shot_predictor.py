...
def predict_shot(request: ShotPredictionRequest) -> ShotPredictionResponse:
    ...
    if ml_probability is None:
        # If ML is unavailable, preserve a useful prediction with Phase 3 rules.
        print("ShotOptix prediction_source=rule_based_fallback")
        ...
        # Use the trained model whenever it returns a valid probability.
        print("ShotOptix prediction_source=ml_model")
        make_probability = ml_probability
        prediction_source = "ml_model"

    # Convert the probability into the full response shown by the frontend.
